...
>) {
  ...
  const rotateDirection = useCallback(
    (currentDirection: Direction, clockwise: boolean): Direction => {
  ...
  const movingMapDark: Record<Direction, string> = {
    ...
    RIGHT:
      "radial-gradient(16.2% 41.199999999999996% at 100% 50%, hsl(0, 0%, 00%) 0%, rgba(255, 255, 255, 0) 100%)",
  };
  ...
  useEffect(() => {
    if (!hovered) {
      const interval = setInterval(() => {
        setDirection((prevState) => rotateDirection(prevState, true));
      }, duration * 1000);
  ...
  return (
    ...
    >
      <div
        className={cn(
          "w-auto text-white z-10 bg-black px-4 py-2 rounded-[inherit] dark:text-black dark:bg-white",
          className
        )}
      >
        {children}
